...
**`dim_player` покрывает всех, но атрибуты только у GM.** Партии включают
соперников, которые не входят в scope загрузки профилей (только гроссмейстера
из titled/GM). Чтобы relationships-тест на fct_games не терял связь для
большинства партий, dim_player строится как UNION всех username из fct_games
с LEFT JOIN к снапшоту профилей (snap_player_profiles) - игроки без профиля
получают NULL в title/location/status но остаются в измерении. Историчность
(SCD2) живёт в snap_player_profiles отдельно; dim_player берёт только
текущую версию (dbt_valid_to IS NULL).
